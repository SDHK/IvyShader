...
FragOut Frag(FragIn fragIn)
{
    ...
    {
        Light addLightUrp = GetAdditionalLight(lightIndex, geomOut.PosWs);
    ...
    //===[环境光照]================================================
    half3 envLight = IvyLight_SH(geomOut.NrmWsFront);
    ...
    // 环境反射探针
    float3 probeReflect = IvyLight_Probe(vecMaps.VecMapReflect, mipMap);
    // 环境贴图反射
    ...
    if (geomOut.IsFront && transmit > 1e-4)
    {
        ...
        half3 refrProbe = IvyLight_Probe(refrWs, mipMap);
