Shader "Ivy/GlassShellEnvTwoPass"
{
    Properties
    {
        [Header(Glass)]
    ...
    SubShader
    {
        ...
        half3 IvyEnvRgb_Glass(half3 dirWs, half lod)
        {
            ...
            return lerp(IvyProbeRgb_Glass(dirWs, lod),
                        IvyPanoRgb_Glass(dirWs, lod),
                        Input_EnvBlend);
        }
        ...
        half3 IvyEnvFire_Glass(half3 nrmWs, half3 viewWs, half lod, half dispersion)
        {
            half3 sumRgb = 0.0;
            half3 sumW   = 0.0;
            ...
            for (int s = 0; s < IvyDef_SpectrumSteps; s++)
            {
                ...
                half  ior = Input_Ior * (1.0 + (t - 0.5) * dispersion * IvyDef_DispersionSpread);
                half3 dir = refract(-viewWs, nrmWs, 1.0 / max(ior, 1.0));

                half3 w = IvySpectrumWeight_Glass(t);
                sumRgb += IvyEnvRgb_Glass(dir, lod) * w;
                sumW   += w;
            }
        ...
        half4 IvyFrag_Glass(IvyVary_Glass vary, bool isFront : SV_IsFrontFace) : SV_Target
        {
            ...
            if (isFront)
            {
                ...
                {
                    refrRgb = IvyEnvFire_Glass(nrmWs, viewWs, envLod, Input_Dispersion);
                }
                else
                {
                    half3 refrWs = refract(-viewWs, nrmWs, 1.0 / max(Input_Ior, 1.0));
                    refrRgb = IvyEnvRgb_Glass(refrWs, envLod);
                }
