...
float IonMath_PingPong(float t, float min, float max)
{
    float range = max - min;
...
/// <param name="max">最大值</param>
/// <returns>返回在指定范围内循环的值</returns>
float3 IonMath_PingPong(float3 t, float3 min, float3 max)
{
...
half3 IonMath_Palette( in half time, in half3 dcOffset, in half3 amp, in half3 freq, in half3 phase)
{
...
/// </summary>
half3 IonMath_RgbToHsv(half3 c)
{
...
/// </summary>
half3 IonMath_HsvToRgb(half3 c)
{
...
half3 IonMath_HsvDelta(half3 stopHsv, half3 refMidHsv)
{
...
half3 IonMath_ApplyHsvDelta(half3 curMidHsv, half3 delta)
{
    half3 o;
